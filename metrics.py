...
def bivariate_loss(V_pred,V_trgt):
    ...
    # Numerator
    result = torch.exp(-((normx/sx)**2 + (normy/sy)**2 - 2*((corr*normx*normy)/sxsy))/(2*(1 - corr**2)))
    # Normalization factor
    denom = 2 * math.pi * (sxsy * torch.sqrt((1 - corr**2)))
    ...
    result = -torch.log(torch.clamp(result, min=epsilon, max=epsilonMax))
